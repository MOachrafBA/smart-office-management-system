...
/**
 * UserDao - Datenbankzugriff für Benutzer
 * 
...
public class UserDao {
        ...
        try (PreparedStatement ps = Db.get().prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    // Für registrierte Benutzer: Passwort aus der Datenbank prüfen
                    // In einer echten Anwendung würde hier BCrypt.checkpw(password, passwordHash) verwendet
    ...
    public List<User> findAll() throws SQLException {
        String sql = "SELECT id, username, email, first_name, last_name, role, is_active, created_at, last_login " +
                     "FROM bwi520.users ORDER BY username";
